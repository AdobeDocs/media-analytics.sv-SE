...
| `s:user:id` | Konfigurera objekt för mobiler, appmätning VisitorID | (Valfritt)<br/><br/>Användaren anger specifikt besökar-ID. |
...
| `l:stream:bitrate` | QoSInfo | (Obligatoriskt)<br/><br/>Det aktuella värdet för strömmens bithastighet (i bps). |

## Felhändelser

| Namn | Datakälla | Beskrivning   |
| ---  | --- | --- |
| `s:event:source` | Media SDK | (Obligatoriskt)<br/><br/>Felets källa, antingen spelarintern eller programnivå. |
...
| Namn | Datakälla | Beskrivning   |
| ---  | --- | --- |
| `s:stream:chapter_sid` | Media SDK | (Obligatoriskt)<br/><br/>Den unika identifierare som är associerad med kapitlets uppspelningsinstans.  <br/> **Obs!** Ett kapitel kan spelas upp flera gånger på grund av åtgärder som användaren utför vid sökning. |
| `s:stream:chapter_name` | ChapterInfo | (Valfritt)<br/><br/>Kapitelns egna namn (dvs. läsbart för människor). |
| `s:stream:chapter_id` | Media SDK | (Obligatoriskt)<br/><br/>Unikt ID för kapitlet. Detta värde beräknas automatiskt utifrån följande formel: <br/>`MD5(video_id) +`<br/>` "_" +`<br/>`chapter_pos` |
| `l:stream:chapter_pos` | ChapterInfo | (Obligatoriskt)<br/><br/>Kapitlets index i kapitellistan (med början från 1). |
